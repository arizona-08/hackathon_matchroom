<template>
  <div class="bg-custom-white min-h-screen py-10 px-4">
    <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
      ...
      <div
        class="bg-custom-white  border border-primary rounded-tr-lg rounded-br-lg rounded-bl-lg shadow-md p-6 md:col-span-2 space-y-8">
        ...
        <div>
          ...
          <button
            ...
            @click="submitReservation" :disabled="!datesValid || !isAvailable">
            Payer maintenant
          </button>
        </div>
      ...
      <div class="bg-custom-white border border-primary rounded-tr-lg rounded-br-lg rounded-bl-lg shadow-md p-6">
        ...
        <div v-if="room">
          <img :src="room.photo_url || 'https://via.placeholder.com/300x200?text=Chambre'"
          ...
          <div class="text-sm space-y-1">
            ...
            <p><strong>Prix par nuit :</strong>
              {{ negotiatedPrice ? negotiatedPrice + ' € (négocié)' : room.price_per_night + ' €' }}
            </p>
...
const totalPrice = computed(() => {
  if (!room.value || numberOfNights.value === 0) return 0
  const pricePerNight = negotiatedPrice.value ?? room.value.price_per_night
  return pricePerNight * numberOfNights.value
})
