<template>
  ...
  const getRelativePosition = (index) => {
    ...
    const relPos = (index - currentIndex.value + positions) % positions;
    return relPos <= positions / 2 ? relPos : relPos - positions;
  };
  ...
  const getPositionClass = (index) => {
    ...
    if (relPos === 1 || relPos === -1 * (cards.value.length - 1)) return 'position-right';
    if (relPos === -1 || relPos === (cards.value.length - 1)) return 'position-left';
    
    // Pour toutes les autres positions
    return relPos > 0 ? 'position-back position-back-right' : 'position-back position-back-left';
  };
  ...
  .carousel-item {
    ...
    width: 65%;
    height: 90%;
    margin: 0 -32.5%; /* Pour centrer les cartes */
  }
  ...
  .carousel-item.position-left {
    transform: translateX(-60%) translateZ(0) rotateY(15deg);
    z-index: 5;
    opacity: 0.8;
  }
